    ...
    <div>
        <transition name="fade">
            <div v-if="$page.flash.success && show"
                 class="mb-8 flex items-center justify-between bg-green-500 rounded max-w-full">
            ...
            <div v-if="($page.flash.error || Object.keys($page.errors).length > 0) && show"
                 class="mb-8 flex items-center justify-between bg-red-500 rounded max-w-full">
                <div class="flex items-center">
                    <svg class="ml-4 mr-2 flex-shrink-0 w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 20 20">
                        <path
                    ...
                    <div v-if="$page.flash.error" class="py-4 text-white text-sm font-medium">{{ $page.flash.error }}
                    </div>
                    <div v-else class="py-4 text-white text-sm font-medium">
                        <span v-if="Object.keys($page.errors).length === 1">There is one form error. {{$page.errors}}</span>
