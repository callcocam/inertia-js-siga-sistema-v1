    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('users.edit', row.id)">
              <img v-if="row.photo" class="block w-5 h-5 rounded-full mr-2 -my-2" :src="row.photo">
              {{ row.name }}
              <icon v-if="row.deleted_at" name="trash" class="flex-shrink-0 w-3 h-3 fill-gray-400 ml-2" />
            </inertia-link>
          </td>
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center" :href="route('users.edit', row.id)" tabindex="-1">
              {{ row.email }}
            </inertia-link>
          </td>
          <td class="border-t">
          ...
          <td class="border-t w-px">
            <inertia-link class="px-4 flex items-center" :href="route('users.edit', row.id)" tabindex="-1">
